...
class MobileApp(MDApp):
    ...
    def press(self):
        ...
        try:
            for n in self.root.m1.text, self.root.m2.text:
                admixture = (((float(self.root.ids.m1.text)
                ...
                self.root.ids.dobavka1.text = str(round(admixture, 2))
        except ValueError:
            pass
        except TypeError:
            pass
        ...
        try:
            for n in self.root.voda.text:
                water = float(self.root.ids.voda.text) * float(self.root.ids.l.text) / 1000
                self.root.ids.voda1.text = str(round(water, 2))
    ...
    def callback(self, instance):
        ...
        if instance.icon == 'water-check':
            try:
                used_water = float(self.root.ids.napravna.text) / float(self.root.ids.l.text) + (
                        float(self.root.ids.vlaga1.text) / 100 * float(self.root.ids.m3.text)
                        + float(self.root.ids.vlaga2.text) / 100 * float(self.root.ids.m4.text)
                        + float(self.root.ids.vlaga3.text) / 100 * float(self.root.ids.m5.text))
                self.root.ids.h2o.text = str(round(used_water, 2))
            except ValueError and TypeError:
                used_water = float(self.root.ids.napravna.text) / float(self.root.ids.l.text) + (
                        + float(self.root.ids.vlaga2.text) / 100 * float(self.root.ids.m4.text)
                        + float(self.root.ids.vlaga3.text) / 100 * float(self.root.ids.m5.text))
